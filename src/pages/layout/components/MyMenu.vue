<template>
  <div class="my-menu" :style="{ width: menu.asideWidth }">
    <el-menu class="border-0" :default-active="route.fullPath" :unique-opened="true" @select="selectFn"
      ...
      <template v-for="(item, index) in asideMenu" :key="index">
        ...
        <el-sub-menu v-if="item.children && item.children.length > 0" :index="item.name">
          ...
          <el-menu-item v-for="(item2, index2) in item.children" :key="index2" :index="item2.link">
            <el-icon>
