    ...
    <el-row>
      <el-table v-loading="loading" :data="tableData" stripe style="width: 100%" height="500">
        ...
        <el-table-column label="操作">
          <template #default="scope">
            <el-button type="primary" link @click="examine(scope.row)">查看</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-row>
    <!-- 查看用户详情 -->
    <FormDrawer ref="formDrawerRef" title="用户详情" confimText="确定" @submit="sure">
...
function handleChoose() {
  ...
  onMounted(async () => {
    ...
    console.log(res2)
    res2.data.forEach((item: Role) => {
      userOptions.push({ value: item.character, label: item.character })
    })
